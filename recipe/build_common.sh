...
# conda gen-bazel-toolchain customizations -- its cxx_builtin_include_dirs
# are only clang's own builtins and unfiltered_compile_flags is empty. Re-
# supply conda's header dir, library dir and the force-linked systemlibs
# that the CPU build gets baked into //bazel_toolchain. -isystem is two
# tokens so each is its own --copt; the crosstool wrapper forwards -isystem
# to nvcc as well.
build --copt=-isystem --copt=${PREFIX}/include
build --host_copt=-isystem --host_copt=${PREFIX}/include
